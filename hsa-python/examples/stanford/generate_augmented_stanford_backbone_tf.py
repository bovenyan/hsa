...
for replicate in range(replication_factor):
    ...
    match = wildcard_create_bit_repeat(dummy_cs.HS_FORMAT()["length"],0x3)
    dummy_cs.set_field(match, "ip_dst", ip_addr, 32-start_ip_mask)
    mask = wildcard_create_bit_repeat(dummy_cs.hs_format["length"],0x2)
    dummy_cs.set_field(mask, "ip_dst", 0, 32-start_ip_mask)
    rewrite = wildcard_create_bit_repeat(dummy_cs.hs_format["length"],0x1)
...
for port in root_tf_ports:
    r = TF.create_standard_rule([port], wildcard_create_bit_repeat(dummy_cs.HS_FORMAT()["length"],0x3),
    ...
    r = TF.create_standard_rule([port + dummy_cs.PORT_TYPE_MULTIPLIER * dummy_cs.INTERMEDIATE_PORT_TYPE_CONST], 
                                wildcard_create_bit_repeat(dummy_cs.HS_FORMAT()["length"],0x3),
