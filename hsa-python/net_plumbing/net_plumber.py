...
class NetPlumber(object):
    ...
    def get_other_end_of_link(self, port):
      '''
      returns the list of port numbers at the other end of link
      '''
    ...
    def print_pluming_network(self, print_flow=False):
      ...
      for table in self.tables.keys():
        ...
        for rule in self.tables[table]:
          print "Rule: %s (match: %s, mask: %s, rewrite: %s)" % \
          (rule.node_id, rule.match,rule.mask,rule.rewrite)
          print "Pipelined To:"
    ...
    def __set_pipeline_dependencies(self, node):
      for port in node.output_ports:
        next_ports = self.get_other_end_of_link(port)
      ...
      for port in node.input_ports:
        previous_ports = self.get_other_end_of_link(port)
